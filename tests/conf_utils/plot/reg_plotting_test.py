...
class Test_calib_plot():
    ...
    def test_boston(self):
        ...
        fig = plot.plot_calibration(significance_lvls,error_rates)
    ...
    def test_boston_flipping(self):
        ...
        # std
        plot.plot_calibration(significance_lvls,error_rates, ax=axes[0,0], title='std')
        # flip x
        plot.plot_calibration(significance_lvls,error_rates, ax=axes[0,1], flip_x=True, title='flip x')
        # flip y
        plot.plot_calibration(significance_lvls,error_rates, ax=axes[1,0], flip_y=True, title='flip y')
        # flip both
        plot.plot_calibration(significance_lvls,error_rates,ax=axes[1,1], flip_x=True, flip_y=True, title='both')
...
class Test_pred_width():
    ...
    def test_boston(self):
        ...
        fig = plot.plot_pred_widths(significance_lvls,pred_widths)
        _save_reg(fig,"Test_pred_width.test_boston")
    
    def test_boston_subset(self):
        pred_widths = metrics.pred_width(boston_preds)
        fig = plot.plot_pred_widths(significance_lvls[10:],pred_widths[10:])
        ax = fig.axes[0]
    ...
    def test_boston_non_std(self):
        ...
        # No flip
        plot.plot_pred_widths(significance_lvls,pred_widths,ax=axes[0],title="standard")
        # Flip
        plot.plot_pred_widths(significance_lvls,pred_widths,ax=axes[1],flip_x=True,title="flip_x")
        # Save
...
class Test_pred_intervals():
    ...
    def test_boston(self):
        fig = plot.plot_pred_intervals(boston_labels,
            boston_preds[:,:,70]
            # ,incorrect_ci='red'
            # , line_cap = 2
            )
